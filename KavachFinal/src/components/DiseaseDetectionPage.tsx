...
export function DiseaseDetectionPage({ onNavigate }: DiseaseDetectionPageProps) {
  ...
  const handleScan = async () => {
    if (!uploadedImage || !selectedAnimalType) {
      alert('Please upload an image and select animal type');
      return;
    }

    setIsScanning(true);
    
    // Simulate VLM analysis with delay
    await new Promise(resolve => setTimeout(resolve, 3000));
    
    // Generate random result (60% healthy, 40% disease)
    const isHealthy = Math.random() > 0.4;
    
    if (isHealthy) {
      setScanResult({
        status: 'healthy',
        confidence: (Math.random() * 0.10 + 0.90).toFixed(2), // 0.90-1.00
        recommendation: 'No immediate action required. Continue routine monitoring and maintain current biosecurity protocols.'
      });
    } else {
      const diseases = {
        Pig: [
          { name: 'African Swine Fever (ASF)', severity: 'High' },
          { name: 'Porcine Reproductive and Respiratory Syndrome (PRRS)', severity: 'High' },
          { name: 'Swine Influenza', severity: 'Medium' },
          { name: 'Classical Swine Fever', severity: 'High' },
          { name: 'Porcine Epidemic Diarrhea', severity: 'Medium' }
        ],
        Poultry: [
          { name: 'Avian Influenza (H5N1)', severity: 'High' },
          { name: 'Newcastle Disease', severity: 'High' },
          { name: 'Infectious Bronchitis', severity: 'Medium' },
          { name: 'Marek\'s Disease', severity: 'Medium' },
          { name: 'Fowl Pox', severity: 'Low' }
        ],
        Cattle: [
          { name: 'Foot and Mouth Disease', severity: 'High' },
          { name: 'Bovine Tuberculosis', severity: 'High' },
          { name: 'Mastitis', severity: 'Low' },
          { name: 'Lumpy Skin Disease', severity: 'High' },
          { name: 'Brucellosis', severity: 'Medium' }
        ]
      };
      
      const animalDiseases = diseases[selectedAnimalType as keyof typeof diseases] || diseases.Cattle;
      ...
      setScanResult({
        ...
        findings: [
          'Visible physical abnormalities or lesions detected in the image',
          'Unusual body posture or behavioral indicators present',
          'Physical symptoms match known disease pattern database',
          'Color and texture analysis shows deviation from healthy baseline'
        ],
        recommendation: `Immediate veterinary consultation strongly recommended. Isolate the affected animal from the herd immediately and implement strict biosecurity measures. This is a preliminary VLM-based assessment and must be confirmed by a licensed veterinarian through physical examination and laboratory tests.`
      });
    }
